...
class Basket {
    ...
    async append(req, res, next) {
        try {
            ...
            const {tourId, quantity} = req.params
            const basket = await BasketModel.append(basketId, tourId, quantity)
            res.cookie('basketId', basket.id, {maxAge, signed})
            res.json(basket)
        } catch(e) {
            next(AppError.badRequest(e.message))
    ...
    async increment(req, res, next) {
        try {
            ...
            const {tourId, quantity} = req.params
            const basket = await BasketModel.increment(basketId, tourId, quantity)
            res.cookie('basketId', basket.id, {maxAge, signed})
            res.json(basket)
        } catch(e) {
            next(AppError.badRequest(e.message))
    ...
    async decrement(req, res, next) {
        try {
            ...
            const {tourId, quantity} = req.params
            const basket = await BasketModel.decrement(basketId, tourId, quantity)
            res.cookie('basketId', basket.id, {maxAge, signed})
            res.json(basket)
        } catch(e) {
            next(AppError.badRequest(e.message))
    ...
    async remove(req, res, next) {
        try {
            ...
            const basket = await BasketModel.remove(basketId, req.params.tourId)
            res.cookie('basketId', basket.id, {maxAge, signed})
            res.json(basket)
        } catch(e) {
            next(AppError.badRequest(e.message))
